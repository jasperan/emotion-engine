...
- **Eval harness**: `emotionsim eval` runs scenario × seed × prompt-variant matrices against the offline stub LLM (`LLM_BACKEND=stub`), aggregating cooperation/emergence metrics and determinism fingerprints; usable as a CI regression gate (`emotionsim eval` in your pipeline)
- **Runtime auto-detect** (pi/local compat): `LLM_BACKEND=auto` probes vLLM → Ollama → stub; `DATABASE_BACKEND=auto` does a real Oracle credential probe and falls back to SQLite (`emotionsim_runtime.db`). `emotionsim doctor` reports the resolved stack (including "Ollama running models on CPU" detection via `size_vram=0`); `emotionsim dev` starts backend + frontend with detected services.
- **Ollama tag resolution**: built-in scenario `model_id`s are resolved against the local server (`gemma3` → `gemma3:4b`, `qwen3.5:27b` → best local `qwen3.5:*`). `OLLAMA_KEEP_ALIVE=30m` keeps models warm across ticks; `OLLAMA_TIMEOUT=300` covers cold-GPU first-token latency (a 90s read timeout previously wasted whole generations on retry).
- **Semantic LLM caching** (`LLM_CACHE_ENABLED`): embed prompts (Ollama nomic-embed-text, offline n-gram fallback) and serve cosine-similar responses; off by default (identity check `is True` so MagicMock settings never engage it); cached content replays through the token stream.
- **Emotion dimensions** (`EMOTION_DIMENSIONS_ENABLED`): valence/arousal in [-1,1] with personality baselines; stress/help/danger events + the LLM's stated emotion (lexicon pull) move them; arousal modulates `should_respond`; state persists in `dynamic_state`.
- **Rumor spread** (`RUMOR_DISTORTION_ENABLED`): `RumorTracker` counts retold-story hops by token overlap; retold messages render distorted in prompts (deterministic per hop; `rumor_spread` events).
- **Dynamic spawning** (`DYNAMIC_SPAWNING_ENABLED`): evacuees arrive on interval/hazard-surge triggers (seeded `DynamicSpawner`), exhausted agents (health ≤ 1 or stress ≥ 9) leave; `agent_spawned` / `agent_left` events; spawned agents persisted.
- **Theory of Mind** (`THEORY_OF_MIND_ENABLED`): agents infer peers' goals/states/intentions from observed actions/messages into a belief store (`What you believe about others` in prompts); observing help nudges relationship trust.
- **Multi-tenant auth** (`AUTH_ENABLED`): `users` table + PBKDF2 password hashes + hashed API keys; `POST /api/auth/register|login` (key shown once, rotated on login); `X-API-Key` header resolves a tenant scope — tenants see own + public scenarios/runs, anonymous sees public only. Auth is optional: CLI/TUI/eval keep anonymous access. **Additive schema**: `tenant_id` columns on scenarios/runs + `users` table; fresh DBs get them via `create_all` — existing Oracle DBs need `ALTER TABLE scenarios ADD tenant_id VARCHAR2(36); ALTER TABLE runs ADD tenant_id VARCHAR2(36);`
