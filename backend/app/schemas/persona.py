...
class Persona(BaseModel):
    ...
    # Opinion dynamics (Tier C - MiroFish integration)
    opinion_vectors: dict[str, float] = Field(default_factory=dict, description="Topic -> stance mapping (-1.0 to 1.0)")
    opinion_bias: float = Field(0.5, ge=0.0, le=1.0, description="Resistance to opinion change (0=easily swayed, 1=immovable)")
    reaction_speed: float = Field(0.5, ge=0.0, le=1.0, description="How quickly opinions shift per interaction")
    influence_level: float = Field(0.5, ge=0.0, le=1.0, description="How much this agent's opinions affect others")
